...
try:
    ...
    with sync_playwright() as p:
        ...
        expect(chat.locator('.dsh-pop [role=menuitem]')).to_have_text(['更改工作区…'])
        chat.get_by_role('menuitem', name='更改工作区…', exact=True).click()
        ...
        chat.get_by_role('menuitem', name='Settings', exact=True).click()
        expect(chat.locator('section #sessionCurrent')).to_be_visible()
        chat.locator('#sessionCurrent .session-more').click()
        expect(chat.locator('.dsh-pop [role=menuitem]')).to_have_text(['更改工作区…'])
        chat.get_by_role('menuitem', name='更改工作区…', exact=True).click()
        expect(chat.locator('.dsh-pop .current')).to_have_text('Settings')
        chat.get_by_role('menuitem', name='无工作区 · 独立会话', exact=True).click()
        expect(chat.locator('#independentSessions #sessionCurrent')).to_be_visible()
