...
try:
    ...
    with sync_playwright() as p:
        ...
        for width, theme in [(1440, 'light'), (960, 'dark')]:
            ...
            expect(page.locator('#balanceCards')).to_contain_text('90% remaining')
            expect(page.locator('#usageChart [data-chart-kind=quota]')).to_have_count(2)
            expect(page.locator('#usageChart [data-chart-kind=quota] .chart-dot')).to_have_count(4)
            expect(page.locator('#balanceCards')).not_to_contain_text('$0')
            page.screenshot(path=str(preview/f'kimi-quota-{theme}.png'), animations='disabled')
            page.locator('[data-view=usage]').click()
            expect(page.locator('#balanceCards')).to_contain_text('Kimi account')
            expect(page.locator('#balanceCards')).to_contain_text('72% remaining')
            assert page.locator('#balanceCards img').evaluate('(img) => img.complete && img.naturalWidth > 0')
            ...
            page.locator('#balanceCards [data-balance="kimi-subscription"]').click()
            page.evaluate("CamelliaI18n.setLanguage('zh-CN')")
            expect(page.locator('#balanceCards')).to_contain_text('剩余 72%')
            expect(page.locator('#usageChart')).to_contain_text('5 小时')
            expect(page.locator('#usageChart')).to_contain_text('30 天观测记录')
            page.locator('#usageChart [data-chart-kind=quota] .chart-dot').last.focus()
            expect(page.locator('.chart-tooltip:visible strong')).to_have_text('90%')
            ...
            expect(page.locator('#balanceCards')).to_contain_text('last successful result')
            expect(page.locator('#balanceCards')).to_contain_text('72% remaining')
