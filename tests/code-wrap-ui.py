...
with sync_playwright() as playwright:
    ...
    for width in [1100, 390]:
        ...
        expect(buttons.first).to_have_text('')
        expect(buttons.first).to_have_attribute('aria-label', 'Word wrap')
        expect(buttons.first.locator('svg')).to_have_count(1)
        unwrapped_icon = buttons.first.locator('path').get_attribute('d')
        ...
        assert buttons.first.locator('path').get_attribute('d') != unwrapped_icon
        expect(buttons.first).to_have_text('')
        ...
        expect(buttons.first).to_have_text('')
        expect(buttons.first).to_have_attribute('aria-label', '自动换行')
        expect(buttons.first).to_have_attribute('title', '自动换行')
