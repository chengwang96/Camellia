  ...
  window.dshDesktop = {
    ...
    conversationCommand:async ({action,payload}) => {
      ...
      if(action==='goal-start') {
        ...
        window.currentGoal={id:'goal-fixture',objective:payload.objective,sessionId:payload.sessionId,phase:'active',armed:true,elapsedMs:0,activeSince:Date.now(),roundsStarted:0};
...
with sync_playwright() as p:
    ...
    for width, height, theme in [(1440, 900, 'light'), (960, 720, 'dark')]:
        ...
        for engine in ['claude', 'codex', 'dsh', 'kimi', 'antigravity']:
            ...
            expect(page.locator('#goalDetails')).to_be_hidden()
            ...
            expect(page.locator('#goalElapsed')).to_contain_text('14h 15m')
            page.locator('#goalBar').screenshot(path=str(preview/f'goal-{engine}-{theme}.png'))
            assert page.evaluate('document.documentElement.scrollWidth <= innerWidth')
            page.locator('#goalPauseBtn').click()
            expect(page.locator('#goalPhase')).to_have_text('Goal paused')
            expect(page.locator('#goalPauseBtn')).to_be_hidden()
            page.locator('#goalResumeBtn').click()
            expect(page.locator('#goalPhase')).to_have_text('Goal in progress')
            page.locator('#goalExpandBtn').click()
            expect(page.locator('#goalFullObjective')).to_have_text(objective)
            ...
            expect(page.locator('#goalReason')).to_be_visible()
            expect(page.locator('#goalReason')).to_contain_text('dataset is unavailable')
            page.locator('#goalResumeBtn').click()
            expect(page.locator('#goalReason')).to_be_hidden()
            ...
            expect(page.locator('#goalResumeBtn')).to_be_hidden()
            page.locator('#goalClearBtn').click()
            expect(page.locator('#goalBar')).to_be_hidden()
