...
用法：
  .\scripts\ollama-switch-account.ps1 setup    # 一次性：保存当前账号为 a，生成 b 的密钥并提示绑定
  .\scripts\ollama-switch-account.ps1 a        # 切换到账号 a
  .\scripts\ollama-switch-account.ps1 b        # 切换到账号 b
  .\scripts\ollama-switch-account.ps1 status   # 显示当前使用哪个账号
  .\scripts\ollama-switch-account.ps1 list     # 列出已保存的账号密钥
#>
param(
...
switch ($Action) {
  'setup' {
    ...
    Write-Host '  以后用  .\scripts\ollama-switch-account.ps1 a / b  即可来回切换。'
  }

  { $_ -in 'a', 'b' } {
    $dir = Join-Path $accountsDir $_
    if (-not (Test-Keypair $dir)) {
      throw "账号 $_ 还没保存密钥。先运行 .\scripts\ollama-switch-account.ps1 setup 完成一次性配置。"
    }
