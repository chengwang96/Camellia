...
> 历史记录：本文中的旧 OpenCode / Ollama 单服务商代理说明已被统一 API 线路池替代，当前入口与配置请以 [配置指南](../configuration.md) 为准。
...
（请求数 / 输入 token / 输出 token）。桌面应用（`src/main/main.js`）已内置集成；不用桌面应用的
设备可单独跑 CLI。
...
- 仓库内置 `src/api/ollama-proxy.js`（核心）+ `scripts/ollama-proxy-cli.js`（独立运行入口）。
...
```powershell
node scripts/ollama-proxy-cli.js            # 默认端口 8788，配置 ~/.dsh/ollama-proxy.json
node scripts/ollama-proxy-cli.js --port 8790 --config <路径>
```
...
> 注：仓库里另有 `scripts/ollama-switch-account.ps1`（基于设备密钥对切换本地 ollama 应用账号），
