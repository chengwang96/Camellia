...
**Python is a single shared interpreter**, configured once at the top of **Settings → Runtime** and used by every harness that calls Python, not by Antigravity alone. When nothing is saved, Camellia detects the first usable Python 3 on `PATH` (`python3`, `python`, and the Windows `py -3` launcher) and shows the detected version; the text field only ever holds a path you saved yourself. Installing Antigravity reuses that interpreter instead of downloading its own CPython, and installs the pinned SDK packages against it. A shared interpreter older than Python 3.10 cannot run the SDK, so the bundled CPython is used instead. Save any Python 3 executable; Camellia reports separately whether it can also import `google.antigravity`, which only Antigravity API mode requires, and it neither modifies the environment nor installs packages into it. The path does not change the dedicated Python environments that external benchmark libraries install. Antigravity still has its own **Antigravity CLI** path for Google subscription mode. If a saved or recorded executable is removed, the engine reports an error instead of silently downloading or choosing a different installation.

**Settings → General → Application updates** checks the project's public GitHub release feed. When a newer version has a package for your platform, **Download and install** downloads it, verifies the release's SHA-256 digest, and confirms before replacing the installation. Installing closes Camellia; your conversations, preferences and downloaded engines are kept. Windows installers run the NSIS wizard in overwrite mode, and macOS builds extract the ZIP and replace the application bundle before restarting. On platforms without a package Camellia can apply in place, the downloaded installer is revealed in the file manager instead. In development the check still works, but in-place installation is limited to packaged builds. No update is downloaded or installed without an explicit click.
