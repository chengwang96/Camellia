...
window.SettingsCharts = (() => {
  ...
  const format = value => new Intl.NumberFormat("en-US", { maximumFractionDigits: 2, notation: Math.abs(value) >= 10000 ? 'compact' : 'standard' }).format(value);
  function line(points, { label, unit = '', percent = false } = {}) {
    ...
    if (!rows.length) return "<div class=\"chart-empty\">No observations yet. Use the app or refresh a balance to start recording.</div>";
    const left = 54, right = 756, top = 20, bottom = 172;
    ...
    const date = at => new Date(at).toLocaleString("en-US", sameDay && rows.length > 1 ? { hour: '2-digit', minute: '2-digit' } : { month: 'numeric', day: 'numeric' });
    const axis = rows.length === 1 ? `<text x="${x(rows[0].at)}" y="199" text-anchor="middle" class="chart-label">${date(rows[0].at)}</text>` : `<text x="${left}" y="199" class="chart-label">${date(rows[0].at)}</text><text x="${right}" y="199" text-anchor="end" class="chart-label">${date(rows.at(-1).at)}</text>`;
    return `<svg class="line-chart" viewBox="0 0 780 205" role="img" aria-label="${escape(label)}">${grid}${rows.length > 1 ? `<polyline points="${coords}" fill="none" class="chart-line"/>` : ''}${dots}${axis}</svg>${rows.length === 1 ? "<p class=\"hint\">One observation recorded. More observations will form a trend.</p>" : ''}`;
  }
  return { line };
})();
