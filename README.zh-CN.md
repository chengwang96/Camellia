...
- **回答末尾展示产物。** 从本轮成功的文件写入操作和回答中的本地文件链接／行内代码路径收集产物，去重并检查文件存在后展示卡片；图片、视频、演示文稿、Markdown 和 HTML 优先展示，超过 4 个文件时折叠其余文件，可展开或收起。“打开方式”提供在 Camellia 内打开或使用系统默认软件打开。支持图片、音视频、PDF、文本以及 DOCX／PPTX／XLSX 内容预览；右侧栏可渲染 Markdown 标题、粗体、表格和代码，以及禁用脚本的沙箱 HTML 页面。Office 预览不还原原始排版、图片、图表或动画，也不重新计算公式，旧版 DOC／PPT／XLS 不支持内置预览。通过脚本生成的文件需在回答中附上路径或链接。共享会话保留各轮产物记录，不扫描整个工作区。
